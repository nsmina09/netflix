...
clickedMovie2 = localStorage.getItem('searchedresult');
console.log(clickedMovie2);
fetch(`https://api.themoviedb.org/3/search/collection?api_key=f95aecc47e9b7acb63aae287f0cd17eb&language=en-US&query=${clickedMovie2}&page=1`)
    .then(data => data.json())
    .then(result => displayClicked2(result))
    .catch(error => console.log(error));

function displayClicked2(result) {
    console.log(result);
    if (result.total_results == 0) {
        htmlData = `<center><p>There is no details for this movie</p></center>`;
        clicked2.innerHTML = htmlData;
